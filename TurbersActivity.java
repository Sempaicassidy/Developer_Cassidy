...
public class TurbersActivity extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_turbers);
        ...
        List<Product> productFromDatabase = dbHelper. getProductsByCategory("Turbers");

        if (productFromDatabase != null && !productFromDatabase.isEmpty()) {
            productList.addAll(productFromDatabase);
            productAdapter.notifyDataSetChanged();
        } else {
            Log.d("TurbersActivity", "No Products found in the database");
        }
    }
}