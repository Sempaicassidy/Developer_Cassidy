...
public class FruitsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        List<Product> productFromDatabase = dbHelper. getProductsByCategory("Fruits");

        if (productFromDatabase != null && !productFromDatabase.isEmpty()) {
            productList.addAll(productFromDatabase);
            productAdapter.notifyDataSetChanged();
        } else {
            Toast.makeText(FruitsActivity.this, "No Products found in the market", Toast.LENGTH_SHORT).show();
        }
    }
}