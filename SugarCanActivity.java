...
public class SugarCanActivity extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_sugar_can);
        ...
        List<Product> productFromDatabase = dbHelper. getProductsByCategory("SugarCan");

        if (productFromDatabase != null && !productFromDatabase.isEmpty()) {
            productList.addAll(productFromDatabase);
            productAdapter.notifyDataSetChanged();
        } else {
            Toast.makeText(SugarCanActivity.this, "No Products found in the market", Toast.LENGTH_SHORT).show();
        }
    }
}